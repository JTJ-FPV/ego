...
{
    const std::string output_path = argc > 1 ? argv[1] : "ego_trajectory_3d.csv";
    constexpr double obstacle_radius = 0.3;

    // Spherical obstacle query. Replace it with a 3D grid or ESDF query as needed.
    const auto occupancy_query = [](const Eigen::Vector3d &point) {
        const bool outside_map = std::abs(point.x()) > 5.0 ||
                                 std::abs(point.y()) > 5.0 ||
                                 std::abs(point.z()) > 5.0;
        const bool inside_sphere = point.norm() < obstacle_radius;
        return outside_map || inside_sphere;
    };
    ...
    config.astar_max_time_ms = 200.0;
    ...
    Eigen::MatrixXd start_state = Eigen::MatrixXd::Zero(3, 3);
    Eigen::MatrixXd target_state = Eigen::MatrixXd::Zero(3, 3);
    start_state.col(0) << -1.0, 0.0, 0.0;
    target_state.col(0) << 1.0, 0.0, 0.0;

    // The initial path crosses the sphere; optimization should route around it.
    Eigen::MatrixXd initial_waypoints(3, 3);
    ...
    if (!success)
    {
        std::cerr << "3D EGO optimization failed\n";
        return 1;
    }
    ...
    for (double t = 0.0; t < duration + 0.5 * sample_dt; t += sample_dt)
    {
        ...
        const Eigen::Vector3d world_position = position;
        const bool occupied = occupancy_query(world_position);
        collision_free = collision_free && !occupied;
        minimum_clearance = std::min(minimum_clearance, world_position.norm());

    }

    std::cout << "3D optimization succeeded\n"
              << "cost: " << final_cost << '\n'
              << "duration: " << duration << " s\n"
              << "minimum sphere-center distance: " << minimum_clearance << " m\n"
