...
Eigen::VectorXd PolynomialSpline::getStateAndPiece(const double &ts, int &piece, const int &flag) const
{
    ...
    if (ts > epsilon) {
        // Locate the trajectory piece containing the requested time.
        for (; N < real_time_vector_.rows(); ++N) {
            const double next_total = total_dur + real_time_vector_(N);
            
            // Compare with tolerance at piece boundaries.
        ...
        // Convert global time to time within the selected piece.
        if (std::abs(total_dur - ts) < epsilon) {
            t = real_time_vector_(N-1); // Exactly at the end of the previous piece.
            N--; // Correct the piece index.
        } else {
            t = ts - total_dur;
        }
    }

    piece = N; // Return the selected piece index.
...
{       
    ...
    // validity
    // assert(piece_num_ == allPoint_.cols() - 1 && "Piece number is not correct!");
                                             ...
                                             double *gradientK_VT, double &costT)
{
    ...
    for(int i = 0; i < piece_num_; ++i)
    {
        ...
        // Propagate the duration gradient through the time mapping.
...
{
    costD = 0.0;
    // A single point has no spacing cost.
    if(points.cols() == 2)
        return;

    // Samples are stored as a dimension-by-(N+1) matrix.
    const Eigen::MatrixXd &ps = points; 
    const int N = ps.cols() - 1; // N+1 samples define N intervals.
    ...
    // Penalize the variance of squared sample spacing.
    const double dquarmean = dsqrs.squaredNorm() / double(N);
    costD = weight_dis * dquarmean;

    Eigen::MatrixXd grad_d_p(ps.rows(), ps.cols());
    grad_d_p.setZero();

    // Compute the gradient with respect to sampled positions.
    for (int i = 0; i <= N; ++i) 
    {
        if (i > 0) 
        {
            // Contribution from the preceding interval.
            grad_d_p.col(i) += weight_dis * (4.0 * dsqrs(i-1) * dps.col(i-1)) / N;
        }
        if (i < N) 
        {
            // Contribution from the following interval.
            grad_d_p.col(i) += weight_dis * (-4.0 * dsqrs(i) * dps.col(i)) / N;
        }
    }

    // Apply the chain rule to coefficients and piece durations.
    ...
    for(int i = 0; i < piece_num_; ++i)
    {
        const double dt = real_time_vector_(i) / K; // Sampling interval within the piece.
        s1 = 0.0;
        for(int j = 0; j <= K; ++j)
        {
            s2 = s1 * s1;
            ...
            s1 += dt;
            // Shared boundaries receive half weight from each adjacent piece.
            if(j != K || (j == K && i == piece_num_ - 1))
                ++id_p;
        }
    }
...
Eigen::MatrixXd PolynomialSpline::getSampleStates(const int &p_num, const int &flag)
{
    const int K = std::max(p_num + 1, 1);
    const int num_whole_traj = piece_num_ * K + 1;
    Eigen::MatrixXd states(dim_, num_whole_traj);
    double t_base = 0.0;
    for(int i = 0; i < piece_num_; ++i)
    {
        const double Ti = getDuration(i);
        for(int j = 0; j < K; ++j)
            states.col(i * K + j) = getState(t_base + Ti * j / K, flag);
        t_base += Ti;
    }
    states.rightCols(1) = getState(getTotalDuration(), flag);
    return states;
